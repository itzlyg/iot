...
public class TSDBJNIConnector {
    ...
    protected long taos = TSDBConstants.JNI_NULL_POINTER;
    ...
    public static void init(Properties props) throws SQLWarning {
        synchronized (LOCK) {
            if (!isInitialized) {
                ...
                setConfigImp(JsonUtil.toJson(map));
    ...
    /**
     * DLL_EXPORT TAOS_RES *taos_schemaless_insert_ttl_with_reqid(TAOS *taos, char *lines[], int numLines, int protocol, int precision, int32_t ttl, int64_t reqid);
     * @param conn
     * @param lines
     * @param type
     * @param precision
    ...
    /**
     *  DLL_EXPORT TAOS_RES *taos_schemaless_insert_raw_ttl_with_reqid(TAOS *taos, char *lines, int len, int32_t *totalRows, int protocol, int precision, int32_t ttl, int64_t reqid);
     * @param conn
     * @param line
     * @param type
     * @param precision
